...
class CompactPowerCard extends CompactPowerCardBase {
  ...
  _renderPvLabelLines() {
    ...
    for (const ln of lines) {
      ...
      if (ln.glow && ln.glow !== "none") path.style.filter = ln.glow;
      group.appendChild(path);
    }
    // Dot on the bus line (same viewBox system -> always centered on the horizontal line)
    const dot = this._pvLabelDot;
    if (dot) {
  ...
  render() {
    ...
    // PV label -> PV center lines, mirror of the device-line geometry (at top).
    ...
    if (this._usePvLabelLines() && pvLabelPositions.length) {
      ...
      pvLabels.slice(0, pvLabelMax).forEach((lbl, idx) => {
        ...
        if (!st) return;
        const startX = pvLabelPositions[idx]?.x;
        if (startX == null) return;
        const numeric = Math.abs(parseFloat(st.state) || 0);
        const color = lbl.color || pvColor;
        const horiz = Math.abs(pvCenterX - startX);
        const corner = Math.min(4, horiz / 2);    // device-line corner radius
        const dirx = pvCenterX >= startX ? 1 : -1;
        // Device-style mirrored path: stub up, Q-bend at busY, horizontal to center
              ...
              (lbl) => html`<div class="overlay-item pv-label-marker" data-index="${pvLabelItems.indexOf(lbl)}" style="left:${lbl.leftPct}%; top:${lbl.topPct}%;">
                <div class="aux-marker pv-label-marker clickable" @click=${() => this._openMoreInfo(lbl.entity || null)}>
                  ${showPvLabelNames && lbl.name
                    ? html`<div class="aux-sub-label label-name" style="margin-top:-2px; color:${lbl.color}; opacity:${lbl.hidden ? 0.35 : lbl.opacity}; font-size:calc(10px * var(--cpc-scale, 1) * var(--cpc-text-scale, 1)); ">${lbl.name}</div>`
                    : ""}
                <ha-icon icon="${lbl.icon}" style="display:block; line-height:0; margin-top:2px; color:${lbl.color}; opacity:1; --mdc-icon-size: calc(18px * var(--cpc-scale, 1)); filter:${allowGlow && lbl.numeric !== 0 ? `drop-shadow(0 0 8px ${lbl.color})` : "none"};"></ha-icon>
